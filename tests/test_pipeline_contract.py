...
class WholeDeclaredGraphTests(unittest.TestCase):
    ...
    def test_exports_outside_declared_consumers_are_explicit(self) -> None:
        report = check_graph(self._declared_nodes())
        self.assertEqual(
    ...
    def test_only_deep_context_persists_directory_identities(self) -> None:
        slices = {
        ...
        self.assertEqual(
            sorted(slices), ["deep_persist_review"]
        )
        self.assertEqual(slices["deep_persist_review"], "source == 'deep_context_review'")

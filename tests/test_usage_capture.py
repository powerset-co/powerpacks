...
hand-computed golden. The unset path must be a true no-op.
"""
...
class TestUsageCaptureUnsetPath(unittest.TestCase):
    def test_no_env_means_no_file_and_untouched_call(self) -> None:
        would_be_log = Path(tempfile.mkdtemp()) / "usage.jsonl"
        ...
        self.assertFalse(would_be_log.exists())  # zero rows anywhere: a true no-op

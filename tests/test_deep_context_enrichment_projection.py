...
class EnrichmentProjectionTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with self.db.connect() as conn:
            self.db.project_parent(
                ParentRow("parent-1", "parent-worth:parent-1", "Jordan Bravo"), conn=conn)
            self.db.project_person(
                PersonRow("person-a", "parent-1", display_name="Jordan Bravo"), conn=conn)
        self.queue_row = {
            "handle": "jordan-bravo",
    ...
    def _params(self) -> research.ResearchRunParams:
        return research.ResearchRunParams(
            input_csv=self.queue, output_dir=self.out,
            manifest=str(self.manifest), db=self.db)
    ...
    def test_inventory_names_exact_paths_and_hashes(self) -> None:
        ...
        self.assertEqual((entry["parent_id"], entry["person_ids"]),
                         ("parent-1", ["person-a"]))
    ...
    def test_running_terminal_and_changed_projection_preserve_human_decision(self) -> None:
        ...
        research._report_progress(
            params, "running", {"total": 1, "completed": 0, "pending": 1, "failed": 0},
            selection={"fingerprint": "selection-1"})
        self.assertEqual(self.db.query("SELECT status FROM stage_state")[0][0], "running")
        self.db.settle_identity("candidate:email:jordan@example.com", "verify")

        research._report_progress(
            params, "research_complete",
            {"total": 1, "completed": 1, "pending": 0, "failed": 0},
            selection={"fingerprint": "selection-1"})
        first_artifacts = self.db.query("SELECT count(*) FROM artifacts")[0][0]
        self.assertEqual(first_artifacts, 2)
        self.assertEqual(self.db.query("SELECT status FROM stage_state")[0][0], "complete")
        ...
        link = self.db.query(
            ...
            "FROM links WHERE row_key='candidate:email:jordan@example.com'")[0]
        self.assertEqual(tuple(link), ("jordan-two", "verify", "yes"))
        self.assertEqual(self.db.query("SELECT count(*) FROM artifacts")[0][0], first_artifacts)
    ...
    def test_failure_transition_projects_error_without_erasing_artifacts(self) -> None:
        ...
        research._report_progress(
            ...
            selection={"fingerprint": "selection-1"}, error="provider failed")
        stage = self.db.query("SELECT status, error FROM stage_state")[0]
        job = self.db.query("SELECT status, error FROM jobs")[0]
    ...
    def test_zero_work_terminal_projects_empty_inventory(self) -> None:
        self._write_queue([])
        research._report_progress(
            self._params(), "research_complete",
    ...
    def test_reconcile_needs_approval_writes_then_projects_without_spend(self) -> None:
        ...
        raw.mkdir()
        subset = [{
            "parent_slug": "jordan-bravo", "person_ids": ["person-a"],
        ...
        with mock.patch.object(reconcile, "current_worth_selection", return_value={
            ...
            node = reconcile.ReconcileDeepResearch(
                verdicts_jsonl=verdicts, overrides_csv=review, people_csv=people,
                facts_dir=facts, raw_dir=raw, manifest=self.manifest,
                out_dir=self.out, queue_csv=self.queue, budget=1.0, approve=False,
                db=self.db)
            node.run()
        ...
        self.assertEqual(payload["artifacts"], [])
        self.assertEqual(self.db.query("SELECT status FROM stage_state")[0][0],
                         "needs_approval")

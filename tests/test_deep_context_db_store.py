...
class DeepContextStoreTransactionsTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.db.project_parent(ParentRow("parent-1", "parent-worth:parent-1"))
        self.db.project_candidate(LinkRow(
            "candidate-1", "parent-1", "candidate-1", "pub",
            machine_action="verify", machine_reason="old machine reason",
    ...
    def test_machine_projection_preserves_human_and_base_columns(self) -> None:
        self.db.settle_identity("candidate-1", "retarget",
                                replacement_url="https://www.linkedin.com/in/jordan-bravo")
        self.db.project_identity((IdentityMachineProjection(
            "candidate-1", machine_action="detach", machine_approved="auto",
            machine_confidence=0.93, machine_reason="fresh judgment",
            machine_judgment="wrong_person", authoritative_detach=1,
        ...
        self.assertEqual((row["machine_action"], row["machine_reason"]),
                         ("detach", "fresh judgment"))
        self.assertEqual((row["decision_action"], row["decision_approved"]),
                         ("retarget", "yes"))
        self.assertEqual(row["replacement_url"],
    ...
    def test_machine_projection_batch_rolls_back_on_unknown_candidate(self) -> None:
        ...
        row = self.db.query(
            "SELECT machine_action, machine_reason FROM links WHERE row_key='candidate-1'"
        )[0]
        self.assertEqual(tuple(row), ("verify", "old machine reason"))

    def test_review_reset_is_atomic_and_preserves_machine_jobs(self) -> None:
        self.db.set_worth("parent-1", "yes", note="keep in network")
        ...
        self.assertEqual((counts.human_worth_cleared, counts.human_identity_cleared),
                         (1, 1))
        parent = self.db.query("SELECT * FROM parents WHERE parent_id='parent-1'")[0]
        link = self.db.query("SELECT * FROM links WHERE row_key='candidate-1'")[0]
        self.assertIsNone(parent["human_worth"])
        self.assertEqual((link["decision_action"], link["replacement_url"]), (None, None))
        self.assertEqual((link["machine_action"], link["machine_reason"]),
                         ("verify", "old machine reason"))

...
class MigrationConservationTests(unittest.TestCase):
    ...
    def _migrate_child_retarget(self, root: Path, other_url: str = "") -> Db:
        ...
        with review.open("w") as handle:
            ...
            if other_url:
                writer.writerow({"public_identifier": other, "person_id": other, "action": "retarget", "approved": "auto",
                                 "new_linkedin_url": other_url})
    ...
    def test_approved_child_retarget_survives_another_members_verdict(self):
        with tempfile.TemporaryDirectory() as directory:
            db = self._migrate_child_retarget(Path(directory))
            rows = db.query("SELECT * FROM links WHERE row_key='candidate:email:casey@example.com'")
    ...
    def test_duplicate_child_retargets_keep_one_approval_not_a_conflict(self):
        with tempfile.TemporaryDirectory() as directory:
            db = self._migrate_child_retarget(Path(directory), "https://linkedin.com/in/casey-example/")
            rows = db.query("SELECT * FROM links WHERE machine_approved='auto'")
            self.assertEqual(len(rows), 1)
            self.assertEqual(rows[0]["row_key"], "candidate:email:casey@work.example")

    def test_different_child_targets_remain_conflicting(self):
        with tempfile.TemporaryDirectory() as directory:
            db = self._migrate_child_retarget(Path(directory), "https://www.linkedin.com/in/different-casey")
            rows = db.query("SELECT * FROM links")
            self.assertEqual(len(rows), 2)
            self.assertTrue(all(row["machine_approved"] is None for row in rows))

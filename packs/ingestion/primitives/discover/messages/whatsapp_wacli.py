...
- 2026-07-26 (binary integrity + honest install flag): the pinned release
  ...
  `logout` report an existing binary as-is (even a stale pin) and a missing one
  raises `PrimitiveBlocked` naming the install path instead of silently pulling
  the ~33MB asset. The `status` exit code now reflects the payload's `status`
  ("ok" -> 0) instead of exiting 1 for a healthy-but-unpaired install.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    auth.add_argument("--no-install", action="store_true", help="never download the pinned wacli binary; use the installed one, or block if none is present")

...
- 2026-07-24 (fan-in subtraction): the fan-in stopped pretending to do identity
  ...
  are deleted (nothing read the file; `deep_context/merge_candidates/cluster_merge_candidates.py`
...
| [`gmail/steps/apply.py`](gmail/steps/apply.py) | Apply step (`run_gmail_apply`, manifest step id `gmail_apply`) — free and local: apply STORED resolutions per account via an in-process `GmailExtractor().apply_resolutions(...)` (gmail/extract_gmail.py), then materialize the merged Gmail people.csv (no Parallel/RapidAPI) | per-account `people.csv`, combined resolutions | per-account resolved CSVs, merged Gmail `people.gmail.csv` |
| [`gmail/util.py`](gmail/util.py) | The typed discovery boundary: `GmailAccount` / `InvalidAccount` / `GmailDiscovery` frozen records with `discovery_from_manifest` as the ONE manifest parse, plus `candidate_people` (unresolved-contact materialization) and shared `GMAIL_IMPORT_PREFIX` | `discover/gmail/manifest.json` + per-account artifacts | — (pure helpers) |
...
| [`messages/util.py`](messages/util.py) | `contacts.csv` column ownership (`MATCH_ANNOTATION_COLUMNS`, `USER_OWNED_COLUMNS`), messages-vertical tolerant field parsers, the deterministic "worth researching" candidate floor (`contact_floor_reason`) and the selection decision on it (`classify_contact`), interaction/last-message readers. The row model itself (`MessageContactRow`) is defined in [`discover/messages/models.py`](../discover/messages/models.py) | — | — (pure helpers) |
...
| [`merge_people.py`](merge_people.py) | Fan-in **and** the deep-context `realize` step: `PeopleMerge` stamps LinkedIn from `directory.csv`, keys each row `linkedin:<slug>` or `candidate:<contact key>`, groups by that key and unions the fields. Applies NO human decisions and drops nobody with a keyable identity. No person identity resolution — that is `deep_context/merge_candidates/cluster_merge_candidates.py` | `--input` per-source `people.csv` files (default: linkedin, gmail, messages — in precedence order), `directory.csv` | `merged/people.csv`, `merged/manifest.json` |

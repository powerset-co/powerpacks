...
deep-researching (named, LLM-ENRICH, not already matched locally) and
...
Filter (default):
  ...
  - matched_person_id is empty (no local-network linkage)
...
def main() -> None:
    ...
    prepare.add_argument("--include-matched", action="store_true",
                        help="Also include rows already matched to a local-network person")

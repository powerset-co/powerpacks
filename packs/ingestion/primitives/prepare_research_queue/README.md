...
Filter + reshape the unified `contacts.csv` into the input CSV consumed by the
Powerpacks `deep_research_contacts` primitive. Stdlib-only.
...
- no existing local-network linkage: no `matched_person_id`, no
...
```bash
# Whole queue:
python packs/ingestion/primitives/prepare_research_queue/prepare_research_queue.py prepare \
...
python packs/ingestion/primitives/deep_research_contacts/deep_research_contacts.py estimate \
  --input .powerpacks/messages/research_queue.csv \
  --processor core2x

python packs/ingestion/primitives/deep_research_contacts/deep_research_contacts.py run \
  --input .powerpacks/messages/research_queue.csv \
  --processor core2x
```

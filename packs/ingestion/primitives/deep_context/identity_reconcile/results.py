...
from packs.ingestion.primitives.deep_context.identity_reconcile.settlement import (
...
def write_overrides(
    ...
    *,
    ...
    verified = sum(
        row.machine_action == "verify" and row.machine_approved == "auto"
        for row in accepted
    )
    detached = sum(
        row.machine_action == "detach" and row.machine_approved == "auto"
        for row in accepted
    )
...
def upsert_retargets(
    ...
    for proposal in proposals:
        ...
        if (
            approved is None
            and proposal.has_reject_fields
            and not (proposal.llm_reject or "").strip()
        ):
        ...
        settlements.append(MachineIdentitySettlement(
            ...
            machine_reject=(
                ...
                if proposal.has_reject_fields
                else None
            ),
            machine_reject_confidence=(
                float(proposal.llm_reject_confidence or 0)
                if proposal.has_reject_fields
                else 0.0
            ),
            machine_reject_reason=(
                proposal.llm_reject_reason or None
                if proposal.has_reject_fields
                else None
            ),
...
def load_tasks_from_snapshot(db: Db) -> list[IdentityTask]:
...
def merge_subset_tasks(db: Db, fresh: list[IdentityTask]) -> list[IdentityTask]:
    replaced = {
        task.parent_id or task.parent_slug for task in fresh
    }
    prior = [
        task for task in load_tasks_from_snapshot(db)
        if (task.parent_id or task.parent_slug) not in replaced
    ]
    return prior + fresh

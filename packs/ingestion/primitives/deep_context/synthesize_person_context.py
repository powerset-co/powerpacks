...
structured profile from the message bodies. JEV then supplies worth and labels.
Heavy reasoning runs on OpenAI; the
...
Saved facts remain reusable even when their raw bundle is no longer local.

Outputs (fixed dir):
  <out-dir>/<person_id>.jsonl   checkpoint: synthesized facts, worth, labels, usage
  <out-dir>/manifest.json       counts + token/cost totals

Changelog:
  2026-09-24: JEV supplies worth and labels after facts synthesis; paid facts
    are reused unless forced, and checkpointed before tagging.
...
from packs.search.primitives.llm_rerank_candidates.jev.client import INPUT_PRICE_PER_MILLION, MAX_CONCURRENCY
...
SYSTEM_PROMPT = (
    ...
    "still count as relationship evidence (a real friendship is "
    "real), but their content stays out of the profile.\n\n"
    "`relationship_category` is ONE slice tag for how I know this person, judged only from "
    "the messages: 'work' (interaction is primarily professional — deals, roles, projects, "
    "studies, investing), 'personal' (friend / social life), 'family', 'service' (a "
    "transactional counterpart — a tradesperson, agent, host, support or logistics contact), "
    "'mixed' (SUSTAINED professional AND personal threads over time — not one stray mention), "
    "'unknown' (too little signal to tell). Pick the dominant pattern; reserve 'mixed' for "
    "genuinely sustained both; with thin evidence say 'unknown' — never infer the category "
    "from the name, employer, or title alone.\n\n"
...
# Record the extraction contract for provenance. Cached facts are reused unless forced.
...
def pending_target_paths(
    ...
    *,
    ...
    """Reuse paid facts until explicitly forced, including old and Maybe dossiers."""
    return [
        path for path in sorted(raw_dir.glob("*.json"))
        if path.name != "manifest.json"
        and (not person_id or path.stem == person_id)
        and (force or not _load_facts_record(facts_dir / f"{path.stem}.jsonl").get("facts"))
    ]
...
class SynthesizePersonContext(Node):
    ...
    def _plan(self) -> SynthesisPlan:
        ...
        owner = load_owner(self.facts_dir.parent / "owner.json") if not self.no_owner else None
        ...
        paths = [] if self.rejudge and not self.force else pending_target_paths(
            self.raw_dir,
            self.facts_dir,
            force=self.force,
    ...
    def _tagging_paths(self, owner: dict[str, Any] | None) -> list[Path]:
        paths = []
        for path in sorted(self.facts_dir.glob("*.jsonl")):
            if self.person and path.stem != self.person:
                continue
            record, bundle, timestamp = self._tagging_inputs(path)
            facts = record.get("facts") or {}
            if not facts:
                continue
            request = build_request(facts=facts, bundle=bundle, owner=owner or {}, reference_date=timestamp[:10])
            if self.rejudge or not facts.get("labels") or not jev_worth.estimate(request, output_dir=self.facts_dir.parent)["cached"]:
                paths.append(path)
        return paths

    def _tagging_inputs(self, path: Path) -> tuple[dict[str, Any], dict[str, Any], str]:
        record = _load_facts_record(path)
        bundle = _load_bundle(self.raw_dir / f"{path.stem}.json")
        timestamp = record.get("updated_at") or datetime.fromtimestamp(path.stat().st_mtime, timezone.utc).isoformat()
    ...
    def estimate(self) -> dict[str, Any]:
        ...
        jev_cost = 0.0
        jev_people = 0
        synthesized_ids = {path.stem for path in plan.paths}
        for path in plan.paths:
            bundle = _load_bundle(path)
            if not bundle.get("messages"):
                continue
            people += 1
            # Facts are not available until extraction; allow one output-sized profile.
            request = build_request(facts={"summary": "x " * 750}, bundle=bundle,
                                    owner=plan.owner or {}, reference_date=now_iso()[:10])
            ...
            for i, b in enumerate(batches):
                ...
                ceiling_batches += 1
        for path in self._tagging_paths(plan.owner):
            if path.stem in synthesized_ids:
                continue
            record, bundle, timestamp = self._tagging_inputs(path)
            request = build_request(facts=record["facts"], bundle=bundle,
                                    owner=plan.owner or {}, reference_date=timestamp[:10])
            jev_cost += jev_worth.estimate(request, output_dir=self.facts_dir.parent)["cost_usd"]
            jev_people += 1
        return {
            ...
            "estimated_cost_floor_usd": estimate_cost_usd(floor_tokens, people * 750, self.model) + jev_cost,
            "estimated_cost_ceiling_usd": estimate_cost_usd(ceiling_tokens, ceiling_batches * 750, self.model) + jev_cost,
    ...
    def execute(self) -> SynthesizePersonContextManifest:
        ...
        # ---- SELECT: only missing facts unless forced. ----------------------
        self.facts_dir.mkdir(parents=True, exist_ok=True)
        plan = self._plan()
        ...
        # ---- CALL: synthesize missing facts before JEV tagging. --------------
        if plan.paths:
            load_env()
            ...
            def on_result(result: dict[str, Any]) -> None:
                ...
                path = self.facts_dir / f"{pid}.jsonl"
                _backup_facts(path)
        ...
        # GPT checkpoints are durable before JEV starts: retrying a failed label
        # request never repeats extraction.
        tagging_paths = self._tagging_paths(plan.owner)
        jev_usage = {"people": 0, "cached": 0, "input_tokens": 0, "output_tokens": 0, "cost_usd": 0.0}
        if tagging_paths:
            load_env()

            async def tag_saved_facts() -> None:
                ...
                async def tag(path: Path) -> None:
                    async with semaphore:
                        ...
                        result = await jev_worth.classify(
                            facts=record["facts"], bundle=bundle, owner=plan.owner or {},
                            reference_date=timestamp[:10], output_dir=self.facts_dir.parent,
                        )
                        record["facts"]["network_worth"] = result["network_worth"]
                        ...
                        usage = result["usage"]
                        jev_usage["people"] += 1
                        jev_usage["cached"] += int(usage["cached"])
                        if not usage["cached"]:
                            for key in ("input_tokens", "output_tokens"):
                                jev_usage[key] += usage[key]
                            jev_usage["cost_usd"] += usage["input_tokens"] * INPUT_PRICE_PER_MILLION / 1_000_000

                for paths in _chunked(tagging_paths, self.chunk_people):
                    await asyncio.gather(*(tag(path) for path in paths))

            asyncio.run(tag_saved_facts())
        ...
        billed_output = tally.tokens["output_tokens"]
        return SynthesizePersonContextManifest(
            ...
            tokens=tally.tokens,
            estimated_cost_usd=estimate_cost_usd(tally.tokens["input_tokens"], billed_output, self.model) + jev_usage["cost_usd"],
            jev=jev_usage,

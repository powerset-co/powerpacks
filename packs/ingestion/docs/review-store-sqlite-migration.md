...
- [x] **P2 write path** (2026-08-05): every session write (apply_decision, apply_worth_decision, guided-retarget settle) routes through the server's `commit_rows` → `ReviewDb.apply_rows` — one transaction commits the state, then the CSVs are exported FROM it; crash between commit and export recovers at boot (`recover_pending_export`, which refuses if the CSV also changed in the window). Gate PASSED: live A/B replay on the mirror — same keep+detach decisions on the db server vs shipped v1.17.3 → final stores identical row-for-row (timestamps masked), CSV == db export, synthetic identical. Note: judge-apply and the other CLI writers still write CSV between sessions (absorbed by import-on-serve) — they move in P4.

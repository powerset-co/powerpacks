...
- Enrich and Done call `/api/status` immediately on load and then every five
  seconds because an external agent/provider can change their manifests. A
...
External handoff changes are visible on the next five-second Enrich/Done poll,

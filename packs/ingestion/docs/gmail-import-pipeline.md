...
- 2026-07-16: Refocused on contact sync only. The import stage is now
  ...
  move to the $deep-setup processing layer (--resolve-legacy is the legacy
  escape hatch), and Modal indexing is no longer part of $import-gmail (it
  stays in $setup and in $deep-setup's finale).
-->
...
pool for the `$deep-setup` processing layer. The import stage is free and
...
- **Identity strategy:** local directory only. People resolved by prior imports
  ...
  `import/gmail/candidates.csv` for `$deep-setup`, which owns Parallel.ai
...
- **Indexing:** no longer part of `$import-gmail`. The Modal index build stays
  in `$setup` and in `$deep-setup`'s finale; new Gmail contacts become
  searchable after one of those runs.
- **Cloud boundary:** none in the canonical import. Provider calls and the
  Modal upload happen only in `$deep-setup` or behind the `--resolve-legacy`
  escape hatch.
...
flowchart TD
    ...
    H -->|Unresolved or cached-negative| Q["import/gmail/candidates.csv<br/>research pool for $deep-setup"]
    ...
    Q -. "identity research, review, indexing" .-> X["$deep-setup processing layer<br/>(Parallel.ai + RapidAPI + Modal)"]
...
| Candidates staging | Post-directory unresolved queues and cached-negative queues are unioned by email into `import/gmail/candidates.csv` (cached negatives flagged in `evidence`). | Every contact worth researching waits for `$deep-setup`; nothing is looked up in-import and nothing is silently dropped. |
...
| Suggest & process tail | A read-only status check reports which sources are imported and how many candidates wait per source, then offers `$deep-setup`. | Indexing is not part of this skill; the Modal build stays in `$setup` and in `$deep-setup`'s finale. |
...
5. Write every still-unresolved contact — including the cached negatives,
   ...
   (`candidate_key` is `email:<addr>`). `$deep-setup` researches each
...
`$deep-setup`. Do not use this flag in the canonical `$import-gmail` flow.
...
| Parallel.ai | Full name, email, an email-domain-derived company guess, and optional context. | Not called by the canonical import — `$deep-setup` (or `--resolve-legacy`) owns this boundary. No Gmail body or subject content. |
| RapidAPI | Accepted LinkedIn URL/public identifier. | Not called by the canonical import — `$deep-setup` (or `--resolve-legacy`) owns this boundary. No Gmail content. |
| Modal | Full merged `people.csv`, including Gmail addresses and interaction metadata. | Not part of `$import-gmail`; the index build happens in `$setup` and in `$deep-setup`'s finale. |
...
- The `$deep-setup` processing layer (candidate research, review, indexing)

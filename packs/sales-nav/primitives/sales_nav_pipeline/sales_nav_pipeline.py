#!/usr/bin/env python3
"""Resumable Sales Nav local/MCP handoff orchestrator.

The script cannot invoke host MCP tools itself. Instead it owns the local run
ledger/state, emits explicit `blocked_mcp_call` instructions for the agent, and
continues once the agent saves MCP responses to files.
"""
...
def block_mcp(lp,l,tool,args_payload,save_to,next_cmd,msg):
    b={"primitive":"sales_nav_pipeline","status":"blocked_mcp_call","message":msg,"mcp_tool":tool,"mcp_args":args_payload,"save_response_to":save_to,"continue_command":next_cmd,"ledger":str(lp)}
...
def cmd_run(args):
    try:
        ...
        # Ingest a supplied MCP response, if present.
        if args.response:
            ...
            return block_mcp(lp,l,"sales_nav_search",payload,save_to,f"python packs/sales-nav/primitives/sales_nav_pipeline/sales_nav_pipeline.py continue --ledger {shlex.quote(str(lp))} --response {shlex.quote(save_to)} --prefer-content", "Run the MCP Sales Nav search, save the JSON response, then continue.")
        if args.require_enriched and not done(l,"ingest_enriched_page"):
            save_to=str(pages/"artifact-full-after-enrich.json")
            return block_mcp(lp,l,"enrich_extended_profiles",{"artifact_id":"<artifact_id from state/search>","set_id":args.set_id},save_to,f"python packs/sales-nav/primitives/sales_nav_pipeline/sales_nav_pipeline.py continue --ledger {shlex.quote(str(lp))} --response {shlex.quote(save_to)} --prefer-content --enriched", "Run enrich_extended_profiles, then get_artifact(include_content=true), save full artifact JSON, and continue.")

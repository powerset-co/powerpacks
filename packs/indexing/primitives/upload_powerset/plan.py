...
# Person-grain namespaces carry allowed_operator_ids and are patched in place;
# entity namespaces are shared corpora that are only ever gap-filled.
PERSON_NAMESPACES = ("people", "summaries", "education")
ENTITY_NAMESPACES = ("companies", "schools")
...
def build_plan(
    *,
    ...
    namespaces = [
        NamespacePlan(logical, namespace_names[logical], tuple(new_to_cloud), tuple(already_in_cloud))
        for logical in PERSON_NAMESPACES
    ]
    entity_ids_by_person = {"companies": company_ids_by_person, "schools": school_ids_by_person}
    for logical in ENTITY_NAMESPACES:
    ...
    private_rows = [row for row in share_rows if row.is_private and row.public_identifier]
    ...
    human_shared_keys = {row.public_identifier for row in share_rows if row.reason == HUMAN_SHARE_REASON}

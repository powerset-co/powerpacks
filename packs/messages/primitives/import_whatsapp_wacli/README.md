...
```bash
uv run --project . python packs/messages/primitives/import_whatsapp_wacli/import_whatsapp_wacli.py status
```

Export from an existing store without syncing:

```bash
uv run --project . python packs/messages/primitives/import_whatsapp_wacli/import_whatsapp_wacli.py export
```

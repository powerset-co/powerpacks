...
Use before `the typed Powerset runner` and role search when a query contains company
...
When many company IDs are returned, pass them to `the typed Powerset runner` so people

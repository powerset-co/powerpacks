...
def dispatch(target: str, *, add_local_db_arg: bool = False) -> None:
    ...
    turbopuffer_client.configure_local_backend(local_args.db)
    if add_local_db_arg:
        rest.extend(["--local-db", local_args.db])

    target_path = PRIMITIVES_DIR / target / f"{target}.py"

...
| Flat person-trait contract at the panel | Shipped | Shared trait extraction (`prompts/traits.txt`); the role-fit expert scores each trait; `jd_fit` on every row; "Fit (Beta)" ordering in the viewer. See `trait-extraction-redesign.md`. |
